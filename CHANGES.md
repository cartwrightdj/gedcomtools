...
- Added `gedcomtools export --to raw-json` / `--to json` for faithful GEDCOM
  5.x and GEDCOM 7 record-tree JSON output.
- Added stdout output support with `--out -` for generated CLI outputs,
  including conversion, repair, raw JSON export, and CSV export.
- Added `--quiet` and `--compact` for script- and agent-friendly modern CLI
  conversion/export workflows.
- CSV stdout mode now emits a JSON envelope containing each generated CSV
  document, since CSV export normally produces multiple files.
- Added the MCP `export_raw_gedcom_json` tool for full GEDCOM 5.x/7 parse-tree
  JSON output.
- Added Bash, Windows batch, and PowerShell examples for both modern CLI usage
  and Python module/API workflows.
- Moved the CLI manual into the Sphinx docs as `docs/cli.md` and rebuilt the
  generated HTML docs.
- Bumped the package, docs, user-agent, GEDCOM 7 package, and gxcli shell
  versions for the 0.8.2 beta refresh.
...
- Added `Gedcom7.parse_bytes()` and routed regular files, URLs, and `.gdz` archive entries through the same decoder so archive loads no longer bypass encoding handling.
- Added regression coverage for UTF-8 BOM files, CP-1252 GEDCOM 7 files, and CP-1252 `.gdz` entries.
- Made GedcomX JSON CLI loading byte-based to avoid Windows default-codepage text reads, and made spec cache text reads/writes explicit UTF-8.
- Made GEDCOM version sniffing tolerant of non-UTF-8 payload bytes while still inspecting only ASCII `HEAD`/`VERS` tags.
- Made the legacy `_gedcom5x` helper tolerate CP-1252 bytes instead of failing on strict UTF-8 decode.
- Removed tracked `.env` from release contents and added `.env` to `.gitignore`.
- Completed release checks: `pyright` passed with zero errors, full pytest passed, package build passed, `twine check` passed, and wheel/sdist artifacts were scanned for local env/sample/log/cache files.
...
## Imported Development Updates

## Conversion fix and v0.8.0b4 build refresh (2026-05-20)
...
- Rebuilt package artifacts for `0.8.0b4`:
  - `dist/gedcomtools-0.8.0b4.tar.gz`
  - `dist/gedcomtools-0.8.0b4-py3-none-any.whl`
...
## Release refresh — v0.8.0b4 (2026-04-25)
...
- Rebuilt Sphinx docs and refreshed the release artifacts for the `v0.8.0b4`
  beta tag.
