...
def _decode_gedcom7_bytes(raw: bytes, source: str) -> tuple[str, Optional[GedcomValidationError]]:
    ...
    GEDCOM 7 requires UTF-8, but files exported on Windows sometimes contain
    CP-1252/Latin-1 bytes.  Falling back makes parsing deterministic while
    still surfacing the encoding problem as a validation warning.
    """
    try:
