...
TOOL_MANUAL: dict[str, dict[str, Any]] = {
    ...
    "export_raw_gedcom_json": {
        "purpose": "Export GEDCOM 5.x or GEDCOM 7 as faithful raw record-tree JSON.",
        "when_to_use": "Give agents a complete parse tree without converting to GEDCOM X.",
        "parameters": {"file_path": "GEDCOM path.", "output_path": "Optional JSON output.", "overwrite": "Allow replacing output.", "include_json": "Return full JSON in MCP response."},
        "caveats": ["Large `include_json=True` responses may be too big for clients."],
    },
    "export_arango_graph": {
...
@mcp.tool()
def export_raw_gedcom_json(
    file_path: str,
    output_path: str | None = None,
    overwrite: bool = False,
    include_json: bool = True,
) -> dict[str, Any]:
    """Export GEDCOM 5.x or GEDCOM 7 as raw record-tree JSON."""
    from gedcomtools.gctool_dataops import _raw_json_document
    from gedcomtools.gctool_load import _load

    path = _ensure_input_file(file_path)
    with _quiet_gedcomtools():
        fmt, obj = _load(path)
        document = _raw_json_document(path, fmt, obj)
    ...
    if output_path:
        out = _ensure_output_file(output_path, overwrite=overwrite)
        out.write_bytes(_dump_json_bytes(document))
        result["output_path"] = str(out)

    if include_json:
        result["raw_json"] = document
